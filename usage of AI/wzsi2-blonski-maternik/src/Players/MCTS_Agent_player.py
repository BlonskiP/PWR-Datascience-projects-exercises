...
agent_player = AgentPlayer(ChessEnv(), PATH='Chess_q_learning\chessLearned50000.99.pt')
custom_player = (lambda b: agent_player.agent_player(b))

def agent_select_playout(legal_moves, board=None, state=None):
    selected_move = custom_player(board)
    return (selected_move, state)
...
def mcts_agent_player(board, strategy=agent_select_playout, display=no_display, move_time_limit=3):
    ...
    while actual_time< move_time_limit :
        plays_counter += 1
        #print(actual_time)
        (playout_next_move, _) = strategy(legal_moves, board=board, state=root)
