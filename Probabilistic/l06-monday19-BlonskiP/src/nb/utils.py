...
def visualize_nb_parameters(history):
    ...
    for pname, pvals in params.items():
        if pname == 'c_logits':
            ...
            newvals = [val[None] for val in pvals]
            fig, axs = plt.subplots(ncols=newvals[0].shape[1], figsize=(15, 5))
            for idx, ax in enumerate(axs):
                for c in range(newvals[0].shape[0]):
                    vals = [v[c][idx] for v in newvals]
