        ...
        for max_f in max_features_values:
            for fold in folds:
                randomForest = RandomForestClassifier(n_estimators=n_estimators,  # how many decision trees inside?
                                                      ...
                                                      max_depth=depth,
                                                      max_features=max_f)
                str_param_temp = str_param+" max_features: "+xstr(max_f) +" max_depth: "+xstr(depth) + " Fold:" + str(fold)
...
def put_to_file(research_result,dataset_name):
    ...
    with pd.option_context("max_colwidth", 1000):
        print(result.head(10).to_latex(index=False,caption="Najlepszy zestaw parametrów dla Random forest "+dataset_name))
    result.to_csv("RF_BestParams_"+filename)
