...
def research_dataset(dataset,dataset_name):
    ...
    str_param = dataset_name + " Bagging est=" + str(n_estimators)
    ...
    for depth in max_depth_values:
        for bootstrap in bootstrap_values:
            for fold in folds:
                bagging = BaggingClassifier(DecisionTreeClassifier(max_depth=depth),max_samples=0.5,max_features=1.0,n_estimators=n_estimators,bootstrap=bootstrap)
                str_param_temp = str_param+" bootstrap: "+xstr(bootstrap) +" max_depth: "+xstr(depth) + " Fold:" + str(fold)
...
def put_to_file(research_result,dataset_name):
    ...
    with pd.option_context("max_colwidth", 1000):
        print(result.head(10).to_latex(index=False,caption="Najlepszy zestaw parametrów dla Bagging "+dataset_name))
    result.to_csv("Bagging_BestParams_"+filename)
